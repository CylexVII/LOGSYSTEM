...
AddEventHandler('esx_policejob:confiscatePlayerItem', function(target, itemType, itemName, amount)
		...
		if targetItem.count > 0 and targetItem.count <= amount then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + amount) > sourceItem.limit then
				...
				if Config.EnableJobLogs == true then
					TriggerEvent('cylex:AddInLog', 'police', 'confiscated', sourceXPlayer.name, amount, sourceItem.label, targetXPlayer.name)
				end
			end
		else
			TriggerClientEvent('esx:showNotification', _source, _U('quantity_invalid'))
	...
	elseif itemType == 'item_account' then
		...
		if Config.EnableJobLogs == true then
			TriggerEvent('cylex:AddInLog', 'police', 'confiscated', sourceXPlayer.name, amount, itemName, targetXPlayer.name)
		end
	...
	elseif itemType == 'item_weapon' then
		...
		if Config.EnableJobLogs == true then
			TriggerEvent('cylex:AddInLog', 'police', 'confiscated', sourceXPlayer.name, amount, ESX.GetWeaponLabel(itemName), targetXPlayer.name)
		end
	end
end)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				...
				if Config.EnableJobLogs == true then
					TriggerEvent('cylex:AddInLog', 'police', 'getSharedInventory', xPlayer.name, count, inventoryItem.label)
				end
			end
		else
			TriggerClientEvent('esx:showNotification', _source, _U('quantity_invalid'))
...
AddEventHandler('esx_policejob:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			if Config.EnableJobLogs == true then
				TriggerEvent('cylex:AddInLog', 'police', 'putStockItems', xPlayer.name, count, inventoryItem.label)
			end
		else
...
ESX.RegisterServerCallback('esx_policejob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	...
	TriggerEvent('esx_datastore:getSharedDataStore', 'society_police', function(store)
		...
		if Config.EnableJobLogs == true then
			TriggerEvent('cylex:AddInLog',"police" ,"add_weapon" ,xPlayer.name)
		end
...
ESX.RegisterServerCallback('esx_policejob:removeArmoryWeapon', function(source, cb, weaponName)
	...
	TriggerEvent('esx_datastore:getSharedDataStore', 'society_police', function(store)
		...
		if Config.EnableJobLogs == true then
			TriggerEvent('cylex:AddInLog',"police" ,"removeArmoryWeapon" ,xPlayer.name)
		end
