	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						if Config.EnableJobLogs == true then
							TriggerServerEvent('cylex:AddInLog', "mecano", "sendBill", GetPlayerName(PlayerId()), GetPlayerName(closestPlayer), _U('mechanic'), amount)
						end
					end
				end
			end, function(data, menu)
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
			    ...
			    if Config.EnableJobLogs == true then
					TriggerServerEvent('cylex:AddInLog', "mecano", "hijack_vehicle", GetPlayerName(PlayerId()))
                end
	...
	elseif data.current.value == 'fix_vehicle' then
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
			    ...
			    if Config.EnableJobLogs == true then
					TriggerServerEvent('cylex:AddInLog', "mecano", "fix_vehicle", GetPlayerName(PlayerId()))
                end
	...
	elseif data.current.value == 'clean_vehicle' then
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
			    ...
			    if Config.EnableJobLogs == true then
					TriggerServerEvent('cylex:AddInLog', "mecano", "clean_vehicle", GetPlayerName(PlayerId()))
                end
	...
	elseif data.current.value == 'del_vehicle' then
		...
		if IsPedSittingInAnyVehicle(playerPed) then
			...
			if GetPedInVehicleSeat(vehicle, -1) == playerPed then
				...
				if Config.EnableJobLogs == true then
					TriggerServerEvent('cylex:AddInLog', "mecano", "del_vehicle", GetPlayerName(PlayerId()))
				end
			else
				...
				if targetVehicle ~= 0 then
					if not IsPedInAnyVehicle(playerPed, true) then
						if vehicle ~= targetVehicle then
							...
							if Config.EnableJobLogs == true then
								TriggerServerEvent('cylex:AddInLog', "mecano", "dep_vehicle", GetPlayerName(PlayerId()))
							end
							if NPCOnJob then
