				...
				if xPlayer.getMoney() >= amount then
					...
					MySQL.Async.execute('DELETE from billing WHERE id = @id', {
						...
						if Config.EnableJobLogs == true then
							TriggerEvent('cylex:AddInLog', xTarget.job.name, 'paybill', xPlayer.name, xTarget.name, amount)
						end
						cb()
					end)
					...
					MySQL.Async.execute('DELETE from billing WHERE id = @id', {
						...
						TriggerClientEvent('esx:showNotification', xPlayer.source, _U('paid_invoice', ESX.Math.GroupDigits(amount)))
							...
							local TargetJob = ""
								...
								TriggerEvent('cylex:AddInLog', TargetJob, 'paybill', xPlayer.name, xTarget.name, amount)
							end
						if xTarget ~= nil then
							TriggerClientEvent('esx:showNotification', xTarget.source, _U('received_payment', ESX.Math.GroupDigits(amount)))
						end
					...
					MySQL.Async.execute('DELETE from billing WHERE id = @id', {
						...
						TriggerClientEvent('esx:showNotification', xPlayer.source, _U('paid_invoice', ESX.Math.GroupDigits(amount)))
							...
							local TargetJob = ""
								...
								TriggerEvent('cylex:AddInLog', TargetJob, 'paybill', xPlayer.name, xTarget, amount)
							end
						if xTarget ~= nil then
							TriggerClientEvent('esx:showNotification', xTarget.source, _U('received_payment', ESX.Math.GroupDigits(amount)))
						end
